...
bool GLShader::LoadFromFile(const std::string& vertexPath, const std::string& fragmentPath) {
    ...
    {
        ServiceLocator::Get<Logger>()->Log("Shader hasnt loaded: reading of shader file failed.", Logger::Level::Error);
        return false;
    }
...
bool GLShader::LoadFromSource(const std::string& vertexSrc, const std::string& fragmentSrc) {
    ...
    {
        ServiceLocator::Get<Logger>()->Log("Shader's sources are empty.", Logger::Level::Error);
        return false;
    }
    ...
    if (!success) {
        ...
        std::stringstream ss;
        ss << infoLog;
        ServiceLocator::Get<Logger>()->Log("Shader linking failed: " + ss.str(), Logger::Level::Error);
        return false;
    }
