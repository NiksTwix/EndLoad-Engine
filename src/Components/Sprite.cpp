...
std::unordered_map<std::string, ECS::ESDL::ESDLType> SpriteService::GetComponentFields(void* component)
{
	...
	Sprite* spr = static_cast<Sprite*>(component);
...
void SpriteService::AddComponentByFields(ECS::EntityID id, std::unordered_map<std::string, ECS::ESDL::ESDLType> fields)
{
	...
	for (auto [fieldName, fieldValue] : fields)
	{
		if (fieldName == "Path") {
			if (auto wu = ECS::ESDL::ValueParser::GetAs<std::string>(fieldValue)) {
				...
				ServiceLocator::Get<Logger>()->Log("Sprite's deserialization error: value's type is not string. Entity " + std::to_string(id) + ".", Logger::Level::Error);
			}
		}
		if (fieldName == "Color") {
	...
	if (m_eSpace != nullptr) m_eSpace->GetRegistry().Add<Sprite>(id, sprite);
	else
	{
