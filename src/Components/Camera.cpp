...
Camera CameraService::CreateCamera(int width, int height, ProjectionType projection_type, float fov, float ortho_size)
{
...
void CameraService::UpdateProjectionMatrix(Camera& cam, int width, int height, ProjectionType type,float fov) {
...
void CameraService::UpdateViewMatrix(Camera& cam, Transform& transform)
{
...
Math::Matrix4x4 CameraService::GetCameraMatrix(Camera& cam)
{
    return cam.Projection*cam.View;
}

void CameraService::UpdateCameraVectors(Camera& cam, Transform& transform)
{
...
void CameraService::UpdateFrustumCulling(Camera& cam)
{
...
void CameraService::UpdateAllCameras(SceneContext& context)
{
    ...
    ECS::View<Transform, Camera> view(context.GetEntitySpace().GetRegistry());
    ...
    TransformService* TS = static_cast<TransformService*>(m_eSpace->GetServiceLocator().GetByComponent<Transform>());
    int index = 0;

    for (auto cam_ptr : cameras) 
    {
        auto global_transform = TS->GetGlobalTransform(entities[index], context);

        cam_ptr->GlobalTransform = global_transform;
...
void CameraService::SetAsCurrent(Camera& cam)
{
	cam.IsCurrent = true;
	CurrentCamera = cam;
}

bool CameraService::IsVisibleAABB(const AABB& aabb, const Camera& camera)
{
...
void CameraService::AddComponentByFields(ECS::EntityID id, std::unordered_map<std::string, ECS::ESDL::ESDLType> fields)
{
    ...
    {
        if (fieldName == "FOV") {
            TrySetNumericField<float>(cam.FOV, fieldValue);
        }
        else if (fieldName == "Near") {
            TrySetNumericField<float>(cam.Near, fieldValue);
        }
        else if (fieldName == "Far") {
            TrySetNumericField<float>(cam.Far, fieldValue);
        }
        else if (fieldName == "OrthoSize") {
            TrySetNumericField<float>(cam.OrthoSize, fieldValue);
        }
        else if (fieldName == "IsCurrent") {
            if (auto current = ECS::ESDL::ValueParser::GetAs<bool>(fieldValue)) {
                cam.IsCurrent = *current;
            }
            else {
                ServiceLocator::Get<Logger>()->Log("Camera's deserialization error: value's type is not boolean. Entity " + std::to_string(id) + ".", Logger::Level::Error);
            }
        }
        else if (fieldName == "Width") {
            TrySetNumericField<int>(cam.Width, fieldValue);
        }
        else if (fieldName == "Height") {
            TrySetNumericField<int>(cam.Height, fieldValue);
        }
            ...
            if (auto wu = ECS::ESDL::ValueParser::GetAs<std::string>(fieldValue)) {
                ...
                ServiceLocator::Get<Logger>()->Log("Camera's deserialization error: value's type is not string. Entity " + std::to_string(id) + ".", Logger::Level::Error);
            }
        }
    }

    if (m_eSpace != nullptr) m_eSpace->GetRegistry().Add<Camera>(id, cam);
    else 
    {
        ServiceLocator::Get<Logger>()->Log("Camera's deserialization error: m_eSpace is nullptr.", Logger::Level::Error);
    }
}
