...
{	//TODO поддержка другой API, а не только OpenGL
	...
	{
		ServiceLocator::Get<Logger>()->Log("Mesh id " + std::to_string(id) + " is invalid.", Logger::Level::Error);
		return;
	}
...
Mesh MeshService::CreateMesh(const std::string& path)
{
...
void MeshService::BuildAABB(Mesh& mesh)
{
	MeshResource* meshr = ServiceLocator::Get<ResourceManager>()->Load<MeshResource>(GetData(mesh.mesh_id).path).get();

	if (meshr == nullptr) 
	{
		ServiceLocator::Get<Logger>()->Log("BuildAABB: mesh id " + std::to_string(mesh.mesh_id) + ", resource is invalid.", Logger::Level::Error);
		return;
	}
...
AABB MeshService::GetTransformedAABB(const Mesh& mesh, Transform& global_transformation)
{
	//OBB
	auto modelMatrix = static_cast<TransformService*>(m_eSpace->GetServiceLocator().GetByComponent<Transform>())->GetModelMatrix(global_transformation);
	...
	for (auto [fieldName, fieldValue] : fields)
	{
		if (fieldName == "Path") {
			if (auto wu = ECS::ESDL::ValueParser::GetAs<std::string>(fieldValue)) {
				...
				ServiceLocator::Get<Logger>()->Log("Mesh's deserialization error: value's type is not string. Entity " + std::to_string(id) + ".", Logger::Level::Error);
			}
		}
	}
	if (m_eSpace != nullptr) m_eSpace->GetRegistry().Add<Mesh>(id, mesh);
	else
	{
		ServiceLocator::Get<Logger>()->Log("Mesh's deserialization error: m_eSpace is nullptr.", Logger::Level::Error);
	}
}
